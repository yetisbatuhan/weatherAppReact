...
const Header = () => {
  ...
  useEffect(() => {
    if (cityApi) {
      axios.get(`https://api.openweathermap.org/geo/1.0/direct?q=${cityApi}&limit=5&appid=${weatherApiKey}`)
        ...
        .then((response) => {
          setWeatherData(response.data); // Veriyi bileşen state'ine kaydet
  ...
  useEffect(() => {
    if (weatherData) {
      // "http://openweathermap.org/img/w/<%= weatherDays.list[i].weather[0].icon %>.png img **************
      axios.get(`https://api.openweathermap.org/data/2.5/weather?lat=${weatherData[0]?.lat}&lon=${weatherData[0]?.lon}&appid=${weatherApiKey}`)
        .then((response) => {
          setCurrentWeather(response.data); // Veriyi bileşen state'ine kaydet
        })
        .catch((error) => {
          console.error('Veri alınamadı:', error);
        });
  ...
  useEffect(() => {
    if (weatherData) {
      // Axios ile bir HTTP isteği göndermek için useEffect kullanabilirsiniz
      axios.get(`https://api.openweathermap.org/data/2.5/forecast?lat=${weatherData[0]?.lat}&lon=${weatherData[0]?.lon}&appid=${weatherApiKey}`)
        .then((response) => {
          setFiveDaysData(response.data); // Veriyi bileşen state'ine kaydet
      ...
      <div className='container mx-auto  my-1 '>
        <div className='grid grid-rows-1 sm:grid-cols-1 md:grid-cols-2 flex'>
          <div className='grid-row-1'>
            <div className='currentWeather flex py-3'>
              <div className='currentWeather-left-col w-full md:w-2/4 '>
                ...
                {currentWeather !== null ? (
                  <h1 className='font-bold text-center md:text-left px-5 py-2 text-[50px] md:text-5xl'>{Math.floor(currentWeather.main.temp - 274) + "°"}</h1>
                ) : (null)}
              </div>
              ...
              {fiveDaysData !== null ? (
                ...
                <div className='grid grid-cols-5 grid-rows-1 '>
                  {fiveDaysData.list.map((data, index) => (
                    index <= 4 ? (
                      <div className='grid-cols-1 px-3 font-bold ' >
                        ...
                        <h6 className='font-bold text-center  px-5 py-2 text-[25px] md:text-3xl'>{Math.floor(data.main.temp - 274) + "°"}</h6>
            ...
            {currentWeather !== null ? (
              <div className='grid grid-rows-2 grid-cols-2 gap-8'>
                ...
                <h3 className='font-bold px-2 text-gray-300'>Real Feel</h3>
                 ...
                 <h3 className='font-bold text-center text-3xl'>{Math.floor(currentWeather.main.feels_like-274)+"°"}</h3>
                </div>
                <div className='grid-cols-1'>
          ...
          <div className='grid-col-1 mt-10 ml-3 '>
            <div className='  border-none border-1 rounded-2xl bg-[#202B3C]'>
              {currentWeather !== null && fiveDaysData !== null ? (
                <ul>
                  <h3 className='pt-4 px-5 font-bold text-gray-400 '>5 Days Forecast </h3>
                  {fiveDaysData.list.map((data, index) => (
                    data.dt_txt.search("12:00") !== -1 ? (
                      <div className=' m-1 border-b-2 border-dashed border-gray-500  '>
                        <h2 className='text-center font-extrabold text-lg'>{Math.floor(data.main.temp - 274) + "°"} </h2>

                        <img className='w-[50px] mx-auto  ' src={`http://openweathermap.org/img/w/${data.weather[0].icon}.png`} alt="Weather Icon" />
                        <li className='text-center font-medium text-gray-400' key={index}>
